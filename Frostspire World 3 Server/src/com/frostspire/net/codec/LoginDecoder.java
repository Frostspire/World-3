...
import com.frostspire.net.login.LoginDetailsMessage;
import com.frostspire.net.login.LoginResponses;
import com.frostspire.net.login.LoginUtils;
import com.frostspire.net.security.IsaacRandom;
...
public final class LoginDecoder extends ByteToMessageDecoder {
	...
	private void decodeRequest(ChannelHandlerContext ctx, ByteBuf buffer) {
		...
		if (request != NetworkConstants.LOGIN_REQUEST_OPCODE) {
			GameLoader.getLogger().info("Session rejected for bad login request id: "+request);
	...
	private void decodeType(ChannelHandlerContext ctx, ByteBuf buffer) {
		...
		if (connectionType != NetworkConstants.NEW_CONNECTION_OPCODE
			...
			GameLoader.getLogger().info("Session rejected for bad connection type id: "+connectionType);
			LoginUtils.sendResponseCode(ctx, LoginResponses.LOGIN_SERVER_OFFLINE);
			return;
		}
	...
	private void decodeLogin(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) {
		...
		if (encryptedLoginBlockSize != buffer.readableBytes()) {
			GameLoader.getLogger().info(String.format("[host= %s] encryptedLoginBlockSize != readable bytes",
		...
		if(buffer.isReadable(encryptedLoginBlockSize)) {
			...
			if(magicId != 0xFF) {
				GameLoader.getLogger().info(String.format("[host= %s] [magic= %d] was rejected for the wrong magic value.",
			...
			if (memory != 0 && memory != 1) {
				GameLoader.getLogger().info(String.format("[host= %s] was rejected for having the memory setting.",
						ctx.channel().remoteAddress()));
				LoginUtils.sendResponseCode(ctx, LoginResponses.LOGIN_REJECT_SESSION);
				return;
			}
			...
			if(securityId != 10) {
				GameLoader.getLogger().info(String.format("[host= %s] was rejected for having the wrong securityId.",
						ctx.channel().remoteAddress()));
				LoginUtils.sendResponseCode(ctx, LoginResponses.LOGIN_REJECT_SESSION);
				return;
			}
