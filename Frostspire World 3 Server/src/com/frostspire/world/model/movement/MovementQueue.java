package com.frostspire.world.model.movement;
...
public final class MovementQueue {
	...
	public void follow() {
		...
		if (character.getPosition().getDistance(followCharacter.getPosition()) <= 1) {
			...
			//If we're in combat, don't dance.
			if(combatFollow) {
				character.getMovementQueue().reset();
				return;
			}
			
			//TODO: "Dance" if we're not in combat.
			if(followCharacter.getMovementQueue().getFollowCharacter() != null
					&& followCharacter.getMovementQueue().getFollowCharacter() == character) {
