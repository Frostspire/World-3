package com.frostspire.world.content.skill;
...
public class SkillManager {
	...
	public SkillManager addExperience(Skill skill, int experience) {
		...
		experience *= skill.getExperienceMultiplier();
		...
		if (newLevel > startingLevel) {
			...
			player.getPacketSender().sendString(skill.getChatboxInterface() + 1, "Congratulations! You have achieved a " + skillName + " level!");
			player.getPacketSender().sendString(skill.getChatboxInterface() + 2, "Well done. You are now level " + newLevel + ".");
			player.getPacketSender().sendString(skill.getChatboxInterface() + 3, "Click here to continue.");
	...
	/*	if(player.getCurrentTask() != null) {
		...
		if(player.getCurrentSkillingTask() != null) {
			player.getCurrentSkillingTask().stop();
		}
		return this;
	}
	...
	public boolean isSuccess(int skill, int levelRequired) {
		int successChance = Misc.getRandom(skill) + (skill / 10);
		
		System.out.println("Success chance: "+successChance);

		if (successChance >= levelRequired) {
			return true;
		}
