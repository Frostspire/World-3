package com.frostspire.world.entity.combat;
...
import com.frostspire.world.entity.combat.formula.DamageFormulas;
import com.frostspire.world.entity.combat.hit.HitDamage;
import com.frostspire.world.entity.combat.hit.HitMask;
import com.frostspire.world.entity.combat.hit.QueueableHit;
import com.frostspire.world.entity.combat.method.CombatMethod;
import com.frostspire.world.entity.combat.method.impl.MagicCombatMethod;
import com.frostspire.world.entity.combat.method.impl.MeleeCombatMethod;
import com.frostspire.world.entity.combat.method.impl.RangedCombatMethod;
import com.frostspire.world.entity.combat.method.impl.bosses.Dragon;
...
import com.frostspire.world.entity.impl.npc.NPCMovementCoordinator.CoordinateState;
...
import com.frostspire.world.model.Locations.Location;
import com.frostspire.world.model.container.impl.Equipment;
import com.frostspire.world.model.movement.RS317PathFinder;
...
public class CombatFactory {
	 ...
	 * Boss combat methods
	 */
	public static final Dragon DRAGON_FIRE_METHOD = new Dragon();
	
	//Add more combat methods here
	//...
	...
	public static CombatMethod getMethod(Character attacker) {
		...
		if(attacker.isPlayer()) {
			...
			if(npc.getId() == 239) {
				if((Locations.goodDistance(npc.getPosition(), attacker.getCombat().getTarget().getPosition(), 1)) && Misc.getRandom(5) >= 3) {
					return DRAGON_FIRE_METHOD;
				}
			}
			
			//Add combat methods here for npcs...
			//Example code:
			/*if(npc.getId() == ZULRAH) {
				return ZULRAH_COMBAT_METHOD;
			}*/
		}
	...
	public static HitDamage getHitDamage(Character entity, Character victim, CombatType type) {
		...
		} else if(type == CombatType.DRAGON_FIRE) {
			damage = Misc.inclusive(1, DamageFormulas.getMagicMaxhit(entity));
		}
	 ...
	 * @param qHit
	 */
	public static void addHitToQueue(QueueableHit qHit) {
	 ...
	 */
	public static void executeHit(QueueableHit qHit) {
		...
		if(target.isPlayer()) {
			...
			if(method.getCombatType() == CombatType.DRAGON_FIRE) {
				if(qHit.getTotalDamage() > 16) {
					p_.getPacketSender().sendMessage("You are badly burnt by the dragon's fire.");
				}
			}
			...
			if(qHit.getTotalDamage() > 0) {
				handleRecoil(p_, attacker, qHit.getTotalDamage());
				handleVengeance(p_, attacker, qHit.getTotalDamage());
			}

			//Prayer effects
			handlePrayerEffects(attacker, p_, qHit.getTotalDamage(), method.getCombatType());
		}
	 ...
	 * @param damage
	 */
	public static void handleVengeance(Player player, Character attacker, int damage) {
	 ...
	 * @param type
	 */
	public static void handlePrayerEffects(Character attacker, Player victim, int damage, CombatType type) {
