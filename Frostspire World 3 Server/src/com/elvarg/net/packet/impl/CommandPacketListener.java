...
public class CommandPacketListener implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
		...
		try {
			...
			if(parts[0].equalsIgnoreCase("spininterface")) {
			...
			if(parts[0].equalsIgnoreCase("fletchinterface")) {
				player.getPacketSender().sendString(2799, "\\n \\n \\n \\n \\n" + "test");
				player.getPacketSender().sendInterfaceModel(1746, 882, 170);
				player.getPacketSender().sendChatboxInterface(4429);
			}
			if(parts[0].equalsIgnoreCase("barinterface")) {
				Smelting.openSmeltInterface(player);
			}
			if(parts[0].equalsIgnoreCase("smithinterface")) {
				Smithing.openSmithingInterface(player, 2349);
			}
